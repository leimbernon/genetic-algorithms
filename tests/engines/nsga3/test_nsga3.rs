...
use genetic_algorithms::traits::{
    ChromosomeT, ConfigurationT, LinearChromosome, OperatorCompat, VectorFitness,
};
use std::borrow::Cow;
...
fn test_nsga3_validate_zero_objectives() {
    ...
    let ga_config = GaConfiguration::default();
    let nsga3 =
        Nsga3Ga::<Dtlz2Chromosome>::new(config, ga_config).with_initialization_fn(|_, _| vec![]);
    let result = nsga3.validate();
    assert!(matches!(result, Err(GaError::InvalidNsga3Configuration(_))));
}
...
fn test_nsga3_validate_missing_reference_points() {
    ...
    let result = nsga3.validate();
    assert!(
        matches!(result, Err(GaError::InvalidNsga3Configuration(msg)) if msg.contains("reference points"))
    );
}
...
fn test_nsga3_validate_custom_reference_point_wrong_dimension() {
    ...
    let result = nsga3.validate();
    assert!(
        matches!(result, Err(GaError::InvalidNsga3Configuration(msg)) if msg.contains("dimension"))
    );
}
...
fn build_test_nsga3(population_size: usize, max_generations: usize) -> Nsga3Ga<Dtlz2Chromosome> {
    use genetic_algorithms::ChromosomeLength;
...
fn test_nsga3_run_invokes_observer_hooks() {
    ...
    let mut nsga3 = Nsga3Ga::<Dtlz2Chromosome>::new(nsga3_config, ga_config)
        ...
        .with_observer(
            observer as Arc<dyn genetic_algorithms::Nsga3Observer<Dtlz2Chromosome> + Send + Sync>,
        )
        .build()
        .expect("build should succeed");
