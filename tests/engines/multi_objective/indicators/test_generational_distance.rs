...
fn test_gd_identical_fronts() {
    ...
    assert!(
        ...
        "GD of identical fronts must be 0, got {}",
        result
    );
}
...
fn test_gd_zdt1_subset() {
    ...
    assert!(
        result > 0.0,
        "GD should be positive for non-identical fronts"
    );
    assert!(
        result < 0.1,
        "GD should be small for ZDT1 near-subset, got {}",
        result
    );
}
...
fn test_gd_dimension_mismatch() {
    ...
    assert!(
        ...
        "Expected InvalidIndicatorConfiguration, got {:?}",
        result
    );
}
