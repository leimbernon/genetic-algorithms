...
use genetic_algorithms::operations::{CreepParams, GaussianParams, Mutation};
...
fn creep_mutation_via_factory_changes_value() {
    ...
    for _ in 0..200 {
        ...
        mutation::factory(Mutation::Creep(CreepParams { step: Some(10.0) }), &mut c).unwrap();
...
fn creep_mutation_via_factory_stays_in_range() {
    ...
    for _ in 0..100 {
        mutation::factory(Mutation::Creep(CreepParams { step: Some(5.0) }), &mut c).unwrap();
        for gene in c.dna() {
...
fn gaussian_mutation_via_factory_stays_in_range() {
    let mut c = build_f64_chromosome(8);
    for _ in 0..200 {
        mutation::factory(Mutation::Gaussian(GaussianParams { sigma: Some(20.0) }), &mut c).unwrap();
        for gene in c.dna() {
            let (lo, hi) = gene.ranges[0];
            assert!(
                gene.value >= lo && gene.value <= hi,
...
fn gaussian_mutation_i32_via_factory() {
    ...
    for _ in 0..200 {
        let before = c.dna().to_vec();
        mutation::factory(Mutation::Gaussian(GaussianParams { sigma: Some(5.0) }), &mut c).unwrap();
        if before.iter().zip(c.dna()).any(|(b, a)| b.value != a.value) {
            changed = true;
            break;
        }
...
fn creep_mutation_step_zero_no_change() {
    ...
    mutation::factory(Mutation::Creep(CreepParams { step: Some(0.0) }), &mut c).unwrap();
...
fn creep_mutation_large_step_stays_in_range() {
    ...
    for _ in 0..100 {
        mutation::factory(Mutation::Creep(CreepParams { step: Some(1e10) }), &mut c).unwrap();
        for gene in c.dna() {
            let (lo, hi) = gene.ranges[0];
            assert!(
                gene.value >= lo && gene.value <= hi,
...
fn gaussian_mutation_sigma_zero_no_change() {
    ...
    mutation::factory(Mutation::Gaussian(GaussianParams { sigma: Some(0.0) }), &mut c).unwrap();
    for (b, a) in before.iter().zip(c.dna()) {
...
fn gaussian_mutation_large_sigma_stays_in_range() {
    let mut c = build_f64_chromosome(3);
    for _ in 0..100 {
        mutation::factory(Mutation::Gaussian(GaussianParams { sigma: Some(1e10) }), &mut c).unwrap();
        for gene in c.dna() {
            let (lo, hi) = gene.ranges[0];
            assert!(
                gene.value >= lo && gene.value <= hi,
...
fn creep_mutation_default_step() {
    ...
    let m = Mutation::Creep(CreepParams { step: None });
...
fn gaussian_mutation_default_sigma() {
    ...
    let m = Mutation::Gaussian(GaussianParams { sigma: None });
    m.mutate(&mut c, &m).unwrap();
    for gene in c.dna() {
        let (lo, hi) = gene.ranges[0];
        assert!(gene.value >= lo && gene.value <= hi);
...
fn gaussian_inline_sigma_controls_noise_magnitude() {
    ...
    let m_small = Mutation::Gaussian(GaussianParams { sigma: Some(0.05) });
    let m_large = Mutation::Gaussian(GaussianParams { sigma: Some(10.0) });
