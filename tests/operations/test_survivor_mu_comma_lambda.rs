...
// In the GA loop, offspring are stamped with `set_age(0)` when created.
// Parents have age > 0 from prior generations (incremented each gen).
// The (mu,lambda) survivor selection keeps only age-0 individuals (the offspring).

#[test]
fn test_mu_comma_lambda_only_keeps_offspring() {
    // Offspring have age=0, parents have age > 0.
    let mut chromosomes = vec![
        make_chromosome(10.0, 3), // parent -- should be discarded
        make_chromosome(5.0, 0),  // offspring (age == 0)
        make_chromosome(8.0, 1),  // parent -- should be discarded
        make_chromosome(3.0, 0),  // offspring (age == 0)
        make_chromosome(7.0, 0),  // offspring (age == 0)
    ];
...
fn test_mu_comma_lambda_no_offspring() {
    // Edge case: all individuals have age > 0 (no offspring). With the age-0
    // convention, this means no one survives — the population becomes empty.
    let mut chromosomes = vec![
    ...
    // No age-0 individuals → all discarded.
    assert_eq!(chromosomes.len(), 0);
}
