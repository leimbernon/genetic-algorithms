...
mod tests {
    ...
    fn test_igd_sparse_approx() {
        ...
        assert!(
            ...
            "Expected IGD=1.0 for sparse approx, got {}",
            result
        );
    }
    ...
    fn test_igd_gt_gd_for_sparse() {
        ...
        let approx = vec![vec![1.0, 2.0]];
        let gd =
            super::super::generational_distance::generational_distance(&approx, &true_front, 2.0)
                .unwrap();
        let igd = inverted_generational_distance(&approx, &true_front, 2.0).unwrap();
        assert!(
            igd > gd,
            "IGD={} must exceed GD={} for sparse front",
            igd,
            gd
        );
    }
