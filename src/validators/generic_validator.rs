...
pub fn fitness_target_is_some(configuration: &GaConfiguration, problem_type: String) -> Result<(), GaError>{
    if configuration.limit_configuration.fitness_target.is_none(){
        return Err(GaError::ConfigurationError(format!(
            "For {} problems, fitness_target must be set.", problem_type)));
    }
    Ok(())
}
...
pub fn chromosome_length_not_bigger_than_alleles<U>(alleles: &[U::Gene], genes_per_chromosome:i32) -> Result<(), GaError>
where
    U:ChromosomeT + Send + Sync + 'static + Clone{
