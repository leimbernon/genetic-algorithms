...
- **D-12:** `pub trait SelfAdaptive: ChromosomeT` — supertrait of `ChromosomeT` (not `LinearChromosome`), parallel to `MultiCaseFitness`. Trait methods: `fn strategy_params(&self) -> &[f64]`, `fn set_strategy_params(&mut self, params: Vec<f64>)`, `fn adapt_strategy_params(&mut self, tau: f64, tau_prime: f64)`. The `adapt_strategy_params` default impl is provided by the trait (calls the log-normal formula using `strategy_params` and `set_strategy_params`).
